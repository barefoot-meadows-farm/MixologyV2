...
const SurpriseMeModal: React.FC<SurpriseMeModalProps> = ({ 
  ...
  useEffect(() => {
    if (!isOpen) return;
    const handleResize = () => {
      // force a re-render to ensure centering (no-op, but could trigger state)
    };
  ...
  return (
    <div 
      className="fixed inset-0 z-[100] flex justify-center items-center p-4 bg-black/50 backdrop-blur-sm"
      style={{ display: 'flex' }}
      onClick={(e) => {
    ...
    >
      <div 
        className="bg-white dark:bg-mixology-dark rounded-lg p-6 shadow-xl max-w-[90vw] w-full max-h-[90vh] mx-auto my-auto flex flex-col items-center justify-center"
        style={{ overflow: 'auto' }}
      >
